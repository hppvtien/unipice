...
//     return $mang;
// }
function permissionShow($user_id)
{
    ...
    foreach ($permissions as $key => $item) {
        if (strpos($item->name, '_index')) {
            $mang = str_replace('_index','.index', $item->name);
        } else {
            $mang  = $item->name;
        }
     
        $sidebars = array_merge($sidebars,[$key => 'get_admin.'.$mang]); 
    }
    return $sidebars;
}
function permissionShowHide($user_id)
{
    $role_id = \DB::table('model_has_roles')->where('model_id', $user_id)->pluck('role_id')->first();
    $gr_permission_id = \DB::table('role_has_permissions')->where('role_id', $role_id)->pluck('permission_id');
